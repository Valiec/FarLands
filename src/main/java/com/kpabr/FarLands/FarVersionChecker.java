...
public class FarVersionChecker {
    public String getNewestVersionID(boolean debug) throws UnknownHostException, IOException
    {
        ...
        if(!debug)
        {
            check = new URL("http://www.kpabr.com/mcmods/FarLands/versionchecknew");
        }
        else
        {
            check = new URL("http://www.kpabr.com/debug/mcmods/FarLands/versionchecknew");
        }
    ...
    public void onUpdateMessage(PlayerEvent.PlayerLoggedInEvent event) throws UnknownHostException, IOException
    {
        try
        {
            ...
            if(versionID>FarLands.versionID)
            {
                event.player.addChatMessage(new ChatComponentText("FarLands is out of date. The newest version is "+versionStr+". Go to the Minecraft Forums thread for updates."));
            }
                else if(versionID<FarLands.versionID)
            {
                event.player.addChatMessage(new ChatComponentText("You are running a development version of FarLands. Bugs may be present."));
            }
